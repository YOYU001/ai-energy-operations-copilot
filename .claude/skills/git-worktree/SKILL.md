...
11. **問使用者要不要 merge、用哪種 merge 方式**（這個專案預設 squash），得到明確同意後才跑 `pr-merge <PR> --strategy squash`。
12. **確認清理乾淨**：`pr-merge` 會自動刪 remote branch；本機再跑一次 `remove <branch>` 清掉本機 worktree（此時應該已經合併，會順利通過乾淨檢查）。
