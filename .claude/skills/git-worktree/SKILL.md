...
執行 `gh pr merge --squash|--merge --delete-branch`，merge 完自動刪 remote branch。**這個子指令跟 `delete-remote` 一樣，不會問使用者任何問題——執行前你必須已經在這個對話輪次裡得到使用者明確同意要 merge**，不能拿之前哪一輪的同意當這次的許可。
...
7. **自己對這個 PR 做 code review**（可以用多角度分析，比照這個專案已經驗證過的方式：抓 diff、分幾個角度找問題、驗證候選發現、只保留可信的），用 `pr-comment` 把每個發現貼上去——**使用者的語言慣好用什麼語言，comment 就用什麼語言**（這個專案預設繁體中文）。
8. **如果使用者也有外部 review 工具的意見（例如 Codex）**，把兩邊的發現彙整起來：對於你判斷清楚「該修/不該修」的，直接處理並說明理由；對於有疑義、兩邊判斷矛盾、或你沒把握的，明確告訴使用者、讓他決定，不要自己隨便定案。
9. **需要修改的部分**：改完、重新跑 `lint-build`、commit、push（PR 會自動更新），再用 `pr-reply` 回覆對應的討論串說明處理結果。
10. **問使用者要不要 merge、用哪種 merge 方式**（這個專案預設 squash），得到明確同意後才跑 `pr-merge <PR> --strategy squash`。
11. **確認清理乾淨**：`pr-merge` 會自動刪 remote branch；本機再跑一次 `remove <branch>` 清掉本機 worktree（此時應該已經合併，會順利通過乾淨檢查）。
