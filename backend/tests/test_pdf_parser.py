"""Tests for Step 10 Sub-step 2 (text/near_empty/scanned detection + metadata shape).

Ported from spike/tests/test_pdf_parser.py (Step 6 Sub-step 2), import path
and fixture-path depth only (backend/tests/ sits at the same depth below the
repo root as spike/tests/ did).
"""
...
def test_near_empty_page_not_classified_as_scanned():
    ...
    # meaningful image content) -- confirmed by manual visual inspection in
    # Step 6. It must be classified as near_empty, not scanned, and must not
    # be routed to OCR.
