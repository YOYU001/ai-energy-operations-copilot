"""Step 10 Sub-step 2: deterministic hashing helpers.

Ported unchanged from spike/hashing.py (Step 6 Sub-step 4). chunk_id must
not depend on a database SERIAL id (it needs to be the same across repeated
ingestion runs of the same input). It is derived from:
...
overwritten) -- see the ingestion service (Sub-step 3) for how that plays
