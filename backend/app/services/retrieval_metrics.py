"""Step 10 Sub-step 5: pure evaluation-metric functions for the retrieval
regression benchmark.

Ported unchanged from spike/retrieval_metrics.py (Step 6 Sub-step 7) --
import path only. This is evaluation/regression tooling, not a production
request-path dependency: nothing in app/main.py or app/services/retrieval.py
imports this module. It exists so backend/scripts/run_retrieval_benchmark.py
and its regression tests can grade retrieval results without a real
database or embedding API call.

All functions operate on the FULL stored `content` of a candidate, never a
truncated preview (a direct, mechanical requirement carried over from the
spike finding that manual review of a 150-char preview produced a wrong
conclusion about one question's top-1 result).
"""

from __future__ import annotations


def document_correctness(candidate_file_name: str, expected_file_name: str) -> bool:
    return candidate_file_name == expected_file_name
...
def evaluate_candidate(candidate: dict, expected_file_name: str, expected_pdf_page: int | None, expected_keywords: list[str]) -> dict:
    """candidate must have keys: file_name, pdf_page_number_start, pdf_page_number_end, content."""
    return {
        "document_correct": document_correctness(candidate["file_name"], expected_file_name),
        "page_correct": page_correctness(candidate["pdf_page_number_start"], candidate["pdf_page_number_end"], expected_pdf_page),
        "content_correct": exact_content_correctness(candidate["content"], expected_keywords),
    }
