"""Step 10 Sub-step 2: text-based PDF page parsing and scanned-page detection.

Ported unchanged from spike/pdf_parser.py (Step 6 Sub-step 2). Scope for this
module only: per-page text extraction via the PDF text layer, a four-way
page classification (text / near_empty / scanned / ocr_failed), and
...
The heading/page-number regexes below are tuned to the layout actually
observed in the Step 6 spike's four documents (Chinese-numeral top-level
headings, numbered subsection headings, arabic/roman printed page numbers).
They return None rather than raising when a new document's layout doesn't
match -- documented as a known scope limitation, not assumed to generalize.
"""
...
# inference beyond this is attempted in this module.
...
def _image_coverage_ratio(page: fitz.Page) -> float:
    ...
    images could over-count coverage; acceptable for this module's coarse
