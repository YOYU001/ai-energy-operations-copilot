"""Step 10 Sub-step 2: chunking strategies, ported from spike/chunker.py
(Step 6 Sub-step 3/8), with only the import path adjusted.

structured_600_100 is the strategy chosen for production use (confirmed
through Step 6 Sub-step 8: table-chunk counts stayed invariant across chunk
sizes). All four strategies are kept here unchanged so the regression
benchmark can still be re-run against the same baselines.
...
Note: the `chunk_id` field produced by this module (e.g.
"{filename}::{strategy}::prose::0004") is a pre-embedding positional
placeholder used only within this module, not the deterministic
content-hash chunk_id used as document_chunks' primary key (see
hashing.compute_chunk_id) -- the real chunk_id is computed downstream, once
the chunk's embedding_content_hash is known (Sub-step 3).
"""
...
from app.services.pdf_parser import PageParseResult
