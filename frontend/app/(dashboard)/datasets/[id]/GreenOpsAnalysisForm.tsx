...
export default function GreenOpsAnalysisForm({
  datasetId,
  initialValue,
}: {
  datasetId: number;
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const outcome = await runGreenOpsAnalysis(datasetId, parsed);
      if (!outcome.ok) {
        setErrorMessage(
          outcome.status === 422
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1">
        <label
          htmlFor="green-ops-max-expected-interval-hours"
          className="text-xs text-foreground/60"
        >
          最大預期時間間隔（小時）
        </label>
        <input
          id="green-ops-max-expected-interval-hours"
          name={PARAM_NAME}
          type="number"
          step="any"
          ...
          // our own errorMessage state for negative/zero input (same fix as
          // CostAnalysisForm.tsx's Sub-step 13.5 bug). `required` is kept
          // (empty-field blocking is fine to leave to the browser), but
          // numeric-range validation is handled entirely in JS above.
          required
          value={inputValue}
          onChange={(event) => setInputValue(event.target.value)}
          placeholder="例如：6（僅為範例，不會自動帶入）"
          aria-describedby="green-ops-max-expected-interval-hours-hint"
          className="rounded-md border border-black/10 px-3 py-1.5 text-sm dark:border-white/10 dark:bg-transparent"
        />
        <p
          id="green-ops-max-expected-interval-hours-hint"
